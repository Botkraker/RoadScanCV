...
out = ROOT / "data" / "own"; (out / "images").mkdir(parents=True, exist_ok=True)
...
while cap.grab():
    if i % step == 0:
        ...
        r = model.predict(img, imgsz=640, conf=a.conf, device=a.device, verbose=False)[0]
        ...
        tasks.append({"data": {"image": f"/data/local-files/?d=images/{name}"}, "meta": {"segment": seg, "t": round(t, 1)},
